...
if key:
    ...
    for idx, row in df.iterrows():
        star = "⭐️" if (
            row["channel_subs"] > 0
            and row["views"] >= 1.5 * row["channel_subs"]
        ) else ""
        cols = st.columns([1, 4, 1, 1, 1, 1])
        cols[0].image(row["thumbnail"], width=120)
        # 채널명 파란색 진한 폰트
        cols[1].markdown(
            f"<span style='font-weight:bold;color:#0057B8;'>"
            f"{row['channelId']}</span>",
        ...
        cols[4].markdown(
            f"<span style='color:{ {'GREAT':'#CCFF00','GOOD':'#00AA00','BAD':'#DD0000','0':'#888888'}[row['label']] };"
