...
def fetch_video_details(video_info):
    ...
    for i in range(0, len(video_info), 50):
        ...
        for it in res["items"]:
            vid = it["id"]
            # publishedAt: 리스트에 없으면 스니펫 default
...
if key:
    ...
    for idx,row in df.iterrows():
        ...
        # 게시일
        pub = row["publishedAt"]
        date_str = pub.strftime("%Y-%m-%d") if pd.notnull(pub) else "N/A"
        ...
        lbl = row["label"]
        html = (
            f"<span style='color:{color_map[lbl]};"
            f"font-weight:bold'>{lbl}</span>"
        )
