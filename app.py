...
def search_videos_global(keyword, max_results, region_code, duration, published_after, published_before):
    ...
    return [
        (item["id"]["videoId"], item["snippet"]["publishedAt"], item["snippet"]["channelId"], item["snippet"]["channelTitle"])
        ...
        for i in resp["items"]:
            vids.append(
                (
                    ...
                    i["snippet"]["channelId"],
                    None,  # we'll fill channelTitle later
                )
            )
...
def fetch_video_details(video_info):
    ...
    for i in range(0, len(video_info), 50):
        ...
        chan_ids = {v[0]: v[2] for v in batch}
        chan_titles = {v[0]: v[3] for v in batch if v[3] is not None}
            ...
            rows.append(
                {
                    ...
                    "channelId": chan_ids[vid],
                    "channelTitle": it["snippet"].get("channelTitle", chan_titles.get(vid, "")),
...
if key:
    ...
    if use_search:
        if not keyword:
            ...
            st.stop()
        vid_info = search_videos_global(keyword, max_res, region, dur, published_after, published_before)
        sub_count = None
    else:
        if not channel_url:
            st.warning("채널 URL을 입력하세요.")
            st.stop()
        cid = channel_url.split("?")[0].rstrip("/").split("/")[-1]
    ...
    sort_option = st.selectbox(
        "정렬 방식",
        ["조회수 내림차순", "조회수 오름차순", "구독자 수 내림차순", "구독자 수 오름차순", "등급별"],
    )
    ...
    for idx, row in df.iterrows():
        ...
        cols[5].markdown(
            f"<span style='color:{ {'GREAT':'#CCFF00','GOOD':'#00AA00','BAD':'#DD0000','0':'#888888'}[row['label']] };font-weight:bold'>{row['label']}</span>",
            unsafe_allow_html=True,
        )
        pub = row["publishedAt"]
